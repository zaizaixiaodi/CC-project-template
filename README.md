...
- **`工作日志/`（历史账）**：一路"曾经发生了什么"。`/done` 时按 `worklog` skill 把当天追加进 `工作日志/<YYYY-MM-DD>.md`（做了什么、关键决策、踩坑、下一步）。**平时不读**，要追溯时你指定某天，AI 才读那一篇——只记 git 记不下的"为什么"，不逐文件列改动。
- **一轮收尾**：`/done` 一步搞定（PLAN.md 完成则归档 + 更新 HANDOFF.md + 写当天工作日志 + 提交推送）。
